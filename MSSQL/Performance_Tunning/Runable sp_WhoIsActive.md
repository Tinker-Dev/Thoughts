# Runable sp\_WhoIsActive

This is an adaptation of the script found here: [http://whoisactive.com/](http://whoisactive.com/)
...
DECLARE @filter sysname
DECLARE @filter_type VARCHAR(10)
DECLARE @not_filter sysname
DECLARE @not_filter_type VARCHAR(10)
DECLARE @show_own_spid BIT
DECLARE @show_system_spids BIT
DECLARE @show_sleeping_spids TINYINT
DECLARE @get_full_inner_text BIT
DECLARE @get_plans TINYINT
DECLARE @get_outer_command BIT
DECLARE @get_transaction_info BIT
DECLARE @get_task_info TINYINT
DECLARE @get_locks BIT
DECLARE @get_avg_time BIT
DECLARE @get_additional_info BIT
DECLARE @find_block_leaders BIT
DECLARE @delta_interval TINYINT
DECLARE @output_column_list VARCHAR(8000)
DECLARE @sort_order VARCHAR(500)
DECLARE @format_output TINYINT
DECLARE @destination_table VARCHAR(4000)
DECLARE @return_schema BIT
DECLARE @schema VARCHAR(MAX)
DECLARE @help BIT

SET @filter = ''
Set @filter_type = 'session'
--Set @filter_type = 'database'
Set @not_filter = ''
Set @not_filter_type = 'session'
Set @show_own_spid = 0
Set @show_system_spids = 0
Set @show_sleeping_spids = 1
Set @get_full_inner_text = 0
Set @get_plans = 0
Set @get_outer_command = 0
Set @get_transaction_info = 0
Set @get_task_info = 0
Set @get_locks = 0
Set @get_avg_time = 0
Set @get_additional_info = 0
Set @find_block_leaders = 1
Set @delta_interval = 0
Set @format_output = 1
Set @output_column_list = '[dd%][session_id][sql_text][sql_command][login_name][wait_info][tasks][tran_log%][cpu%][temp%][block%][reads%][writes%][context%][physical%][query_plan][locks][%]'
Set @sort_order = '[start_time] ASC'
Set @return_schema = 0
